...
def main():
    Washington = input("Enter a state to retrieve bills: ")
    bills = retrieve_state_bills(Washington)
    print(f"Bills for {Washington}:")
    for bill in bills:
        print(bill)
...
def retrieve_state_bills(state):
    ...
    for bill in data["masterlist"]:
        bill_details_url = f"https://api.legiscan.com/?key={API_KEY}&op=getBill&id={bill['bill_id']}"
        bill_details_response = urllib.request.urlopen(bill_details_url)
